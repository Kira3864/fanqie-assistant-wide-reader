...
(function (vue, moment) {
  ...
  class ChapterCache {
    ...
    retainWindow(currentItemId, orderedItemIds) {
  ...
  async function fetchChapter(itemId, _retry) {
    var _a, _b;
  ...
  async function getChapter(itemId) {
    return chapterCache.get(itemId, () => fetchChapter(itemId));
  }
  function retainChapterCache(currentItemId, orderedItemIds) {
  ...
  function buildReadingProgressPayload(context, now = Date.now()) {
    const officialOrder = Number(context.chapterOrder);
    const catalogOrder = Number.isInteger(context.chapterIndex) && Number(context.chapterIndex) >= 0 ? Number(context.chapterIndex) + 1 : 0;
    ...
    return {
      ...
      read_progress: order,
      index: order,
      read_timestamp: String(Math.floor(now / 1e3)),
  ...
  async function syncReadingProgress(context, requester = getDefaultRequester()) {
  ...
  function calculateSpreadOffset(spread, spreadStep) {
    if (spread <= 0) return 0;
    return -Math.max(0, spread) * Math.max(1, spreadStep);
  }
  function buildColumnPageMap(totalColumns, starts) {
    const safeTotal = Math.max(1, totalColumns);
    const normalized = starts.map((start) => ({ ...start, startColumn: Math.max(0, Math.min(safeTotal - 1, start.startColumn)) })).sort((left, right) => left.startColumn - right.startColumn);
    if (normalized.length === 0) return [];
    const pages = [];
    normalized.forEach((start, index) => {
      var _a;
      const nextStart = ((_a = normalized[index + 1]) == null ? void 0 : _a.startColumn) ?? safeTotal;
  ...
  function calculateChapterBreakFill(offsetTop, columnHeight) {
  ...
  function syncWideReaderContinuation(currentItemId, continuation) {
  ...
  function appendChapter(article, itemId, title, source, continuation) {
    if (continuation) {
    ...
    heading.className = continuation ? "fqa-wide-chapter-heading fqa-wide-continuation" : "fqa-wide-chapter-heading";
    ...
    if (continuation) {
      for (let index = startIndex; index < article.children.length; index += 1) {
        ...
        child == null ? void 0 : child.classList.add("fqa-wide-continuation");
      }
    }
  }
  function appendTerminalPage(article) {
  ...
  function measureLayout(frame, article) {
    ...
    const chapterStarts = [...article.querySelectorAll("[data-chapter-id]")].map((heading) => ({
    ...
    const terminal = article.querySelector("[data-terminal-page]");
    const terminalColumn = terminal ? Math.round(Math.max(0, terminal.offsetLeft) / columnStep) : null;
    const continuationStart = ((_a = chapterStarts[1]) == null ? void 0 : _a.startColumn) ?? terminalColumn;
  ...
  function alignContinuationChapters(frame, article) {
    const frameTop = frame.getBoundingClientRect().top;
    const columnHeight = frame.clientHeight;
    article.querySelectorAll(".fqa-wide-chapter-heading.fqa-wide-continuation").forEach((heading) => {
      const chapterBreak = heading.previousElementSibling;
      if (!(chapterBreak == null ? void 0 : chapterBreak.classList.contains("fqa-wide-chapter-break"))) return;
  ...
  async function openAccountMenu(current, anchor) {
    ...
    (_b = anchor.parentElement) == null ? void 0 : _b.append(menu);
    if (summary) {
      try {
  ...
  function formatAccountSummary(user) {
    if (!user) return "账户";
    if (user.read_book_num === void 0 || user.read_book_time === void 0) return user.username;
    return `${user.username} · 阅读 ${user.read_book_num} 本 · ${formatReadingTime$1(user.read_book_time)}`;
  }
  async function withTimeout(task, timeoutMilliseconds) {
    let timer;
  ...
  function formatReadingTime$1(milliseconds) {
    const safeMilliseconds = typeof milliseconds === "bigint" ? milliseconds : BigInt(Math.max(0, Math.trunc(milliseconds)));
    const totalMinutes = safeMilliseconds / 60000n;
  ...
  function createTextChapterSource(chapter, chapterTitle) {
    var _a;
    if (typeof (chapter == null ? void 0 : chapter.content) !== "string") return null;
    ...
    let source = documentNode.body.querySelector("article") ?? documentNode.body;
    ...
    for (let index = 0; index < Math.min(2, source.childNodes.length); index += 1) {
      ...
      if ((_a = child == null ? void 0 : child.innerHTML) == null ? void 0 : _a.includes(chapterTitle)) {
        child.remove();
        break;
      }
    }
    ...
    container2.append(source);
    return container2;
  }
  async function preloadWideReaderContinuation(itemId, book) {
    ...
    const next = currentIndex >= 0 ? chapters[currentIndex + 1] : void 0;
    if (!next) {
  ...
  async function insertContent() {
    ...
    if (enhancedContent) {
      ...
      const chapterList = (currentBook == null ? void 0 : currentBook.chapter_list) ?? [];
      const chapterIndex = chapterList.findIndex((item) => item.item_id === itemId);
      const initialChapter = (_p = pageState == null ? void 0 : pageState.reader) == null ? void 0 : _p.chapterData;
      const initialStateMatches = String((initialChapter == null ? void 0 : initialChapter.itemId) ?? "") === itemId;
      if (chapterBookId) {
        void syncReadingProgress({
          bookId: String(chapterBookId),
          itemId,
          chapterIndex,
          chapterOrder: ((_q = chapter.novel_data) == null ? void 0 : _q.order) ?? ((_r = chapter.novel_data) == null ? void 0 : _r.chapter_order) ?? ((_s = chapter.novel_data) == null ? void 0 : _s.real_chapter_order) ?? (initialStateMatches ? initialChapter == null ? void 0 : initialChapter.order : void 0),
          genre: ((_t = chapter.novel_data) == null ? void 0 : _t.genre) ?? ((_u = chapter.novel_data) == null ? void 0 : _u.gender) ?? (initialChapter == null ? void 0 : initialChapter.genre) ?? 0
        });
      }
    }
  }
  ...
  const BOOKSHELF_HOVER_WIDTH = 280;
  const BOOKSHELF_HOVER_HEIGHT = 360;
  const BOOKSHELF_HOVER_GAP = 12;
  const VIEWPORT_MARGIN = 8;
  function computeBookshelfHoverLayout(anchor, viewport) {
