...
目前的界面改动都是直接操作 DOM，因为改的量还很小，插几个节点就够了。Vue 已经引入并配好构建（`@vitejs/plugin-vue`），等做到短剧播放、用户详情、评论这类大型界面时会用 Vue 写，但现在还没到那一步。
...
如果后续同源接口无法满足脚本需求，会考虑使用 `GM_cookie` 权限。但我们确保您的凭据不会被发送到番茄小说及其相关平台以外的任何第三方。  
